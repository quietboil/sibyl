...
- Some of the LOB tests need text files the the expected content. Those can be found in `etc/media` and copied into `MEDIA_DIR`.
...
- Tests that connect to the database use environment variables to identify the database, user and password - DBNAME, DBUSER and DBPASS respectively. These variables should be set before executing `cargo test`.
