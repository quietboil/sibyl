...
Only `read` method is implemented and even it uses `DBMS_LOB.READ` workaround as both `OCILobRead2`
and deprecated `OCILobRead` behave erratically in nonblocking mode (they work just fine in blocking
mode).
*/
...
impl<'a> LOB<'a,OCIBLobLocator> {
    ...
    */
    ...
    /*
        ...
        If successful, this function returns the number of bytes that were read from LOB and appended to `buf`.
