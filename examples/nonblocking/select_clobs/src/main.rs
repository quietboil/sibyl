...
async fn main() -> sibyl::Result<()> {
    ...
    let rows = stmt.query((3000, 10000, ())).await?;
    ...
    while let Some(row) = rows.next().await? {
        ...
        let text_len = text.len().await?;

        let mut content = String::new();
        let content_len = text.read(0, text_len, &mut content).await?;

        println!("{id}: [{text_len} / {content_len} / {}] = {name}", content.len());
    }
    Ok(())
}